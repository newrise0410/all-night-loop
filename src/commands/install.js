...
import { loadSkill, bundle } from '../skill.js';
...
export function install(argv) {
  ...
  const ctxBase = { skill, bundleText: bundle(skill), root };
  ...
  for (const a of targets) {
    ...
    for (const item of a.plan({ ...ctxBase, scope })) {
  ...
  log(c.dim('다음: ') + 'anl init' + c.dim('  으로 loop/SPEC.md 를 만든 뒤 직접 채운다.'));
}
